	...
	process {
		if ($PSCmdlet.ShouldProcess($identity)) {
			try {
				#get devices tickets
				if ($serialNumber -ne "") {
					$buildTickets = Get-FreshAssetsTickets -API_Key $API_Key -serialNum $serialNumber
					$identity = (Get-FreshAsset -API_Key $API_Key -serialNum $serialNumber).name
				}
				elseif ($identity -ne "") {
					$buildTickets = Get-FreshAssetsTickets -API_Key $API_Key -name $identity
				}
				else {
					#input validations
				...
				| Where-Object {
					foreach ($pattern in $DeviceManagmentDefaultConfig.Deployment.buildTicketNamePatterns) {
