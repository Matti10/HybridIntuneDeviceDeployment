	...
	process {
		try {
			if ($PSCmdlet.ShouldProcess("")) {
				$securePassword = ConvertTo-SecureString -String (Get-AzKeyVaultSecret -VaultName $DeviceDeploymentDefaultConfig.Security.KeyVaultName -Name $DeviceDeploymentDefaultConfig.Security.elevatedPassword_KeyVaultKey -AsPlainText) -AsPlainText -Force
			}
			else {
